...
describe("Test validation of bowling strike sequences", () => {
    test("Will not approve of negative rolls", () => {
        ...
        expect(bowling_logic_1.validSequenceOfRolls(rolls)).toBeFalsy();
    });
    test("Catches cases where a regular frame has to many points", () => {
        let rolls = [10, 0, 5, 5];
        expect(bowling_logic_1.validSequenceOfRolls(rolls)).toBeTruthy();
        rolls = [10, 1, 5, 5];
        expect(bowling_logic_1.validSequenceOfRolls(rolls)).toBeFalsy();
    });
